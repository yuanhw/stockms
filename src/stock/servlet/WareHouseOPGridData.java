...
public class WareHouseOPGridData extends HttpServlet{
	...
	private void OpInsert(PrintWriter out, HttpServletRequest request){
		try{
			...
			List<WareHouse> whlist=JSON.parseArray(jsonArrayStr, WareHouse.class);
			if(WareHousePFactory.newInstence().doCreate(whlist)){
				out.write("插入成功！");
			}else{
				out.write("插入失败！");
			}
	...
	private void OpUpdate(PrintWriter out, HttpServletRequest request){
		try{
			...
			String jsonArrayStr=request.getParameter("data");
			List<WareHouse> whlist=JSON.parseArray(jsonArrayStr, WareHouse.class);
			if(WareHousePFactory.newInstence().doUpdate(whlist)){
				out.write("更新成功！");
			}else{
				out.write("更新失败！");
			}
	...
	private void OpDelete(PrintWriter out, HttpServletRequest request){
		try{
			...
			while(iter.hasNext()){
				...
				list.add(jobj.getString("wid"));
			}
			if(WareHousePFactory.newInstence().doDelete(list)){
				out.write("删除成功！");
			}else{
				out.write("删除失败！");
			}
	...
	private void OpFindAll(PrintWriter out, HttpServletRequest request){
		try{
			...
			List<WareHouse> list=WareHousePFactory.newInstence().findAll();
			List<WareHouse> sublist=null;
