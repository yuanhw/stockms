...
<html>
  ...
  <body>
   ...
   <a id="btn1" class="easyui-linkbutton" data-options="iconCls:'icon-add'">增加产品</a>
   <a id="btn3" class="easyui-linkbutton" data-options="iconCls:'icon-remove'">删除产品</a>
   	<a id="btn4" class="easyui-linkbutton" data-options="iconCls:'icon-cancel'">退出</a>
   	<a id="btn5" class="easyui-linkbutton" data-options="iconCls:'icon-save'">保存</a>
   </div>
   <div id="instock" style="border:1px solid #abcdef;">
   		<div style="text-align:center;border:1px solid #abcdef;padding:3px;">
   			<span style="font-size:32px;">入库单</span>
   			<span style="margin-left:300px;">编号：
   				<input id="inid" class="easyui-textbox" style="width:100px;color:red;" data-options="disabled:true"/>
   			</span>
   		</div>
   		<div style="border:1px solid #abcdef;padding:3px;">
   			<span>收货仓库：<input id="ck" class="easyui-combobox"/></span>
   ...
   $(document).ready(function(){
	   ...
	   //日期框默认当前日期
	    ...
	    //入库单编号
	    var inid=getId();
	    $("#inid").textbox('setValue',inid);
	    //加载产品信息
	    var productdata=getComboData("prodata");
	    //定义网格并且加载数据
	    var column0=getDefColumns(productdata);
	    var id="dg";
	    fn_def_datagrid(id,column0);
		/*按钮点击事件*/
		//增加产品按钮
		...
		$("#btn5").bind('click',function(){
		   ...
		   var inid=$("#inid").textbox('getValue');
		   var whid=$("#ck").combobox('getValue');
		   var date=$("#date").datebox('getValue');
		   var operator=$("#uid").text();
		   var data={'inid':inid,'whid':whid,'date':date,'operator':operator,'rows':rows};
		   var url="../servlet/InStockOPData";
		   if(whid!=null&&whid!=''){
			   fn_Save(id,data,url);
		   }else{
