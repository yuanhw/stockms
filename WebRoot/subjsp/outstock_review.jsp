  ...
  <body>
   <div id="main">
   	<div id="tools" style="padding:5px;border:1px solid #abcdef;">
   		<label>选择出库单状态：</label><input id="type" class="easyui-combobox">
   ...
   $(document).ready(function(){
	   ...
	   def_datagrid("dg2","出库产品信息",columns2);
	   load_datalist(id,"未审核","../servlet/OutStockOPData");
	   //审核单据
   ...
   function fn_op(state){
	   load_datalist("dg",state,"../servlet/OutStockOPData");
	   $("#dg2").datagrid("loadData",[]);
   }
   function fn_sh(){
	   var rs=$("#dg").datagrid('getSelected');
	   $.ajax({       
			url: "../servlet/OutStockReview",
			data:{"optype":"sh","outid":rs.outid},
   ...
   function fn_del(){
	   ...
	   $.ajax({       
			url: "../servlet/OutStockReview",
			data:{"optype":"delete","outid":rs.outid},
			async: false, method: 'post',    
			success: function(data) {
				if(data=='success'){
					$("#dg").datagrid('deleteRow',index);
  ...
  function getDefColumns1(){
   return columnArray1=[[
    			{field:'id',title:'序号',width:20,checkbox:true,sortable:true,align:'center'},
 				{field:'outid',title:'出库单编号',width:100,align:'center'},
